...
run_with_adversaries() {
	log_dir="logs/eval_correlation/with_adversaries/tradeoff_${1}/asr_${2}"
	mkdir -p $log_dir
	python -u eval_correlation.py \
		...
		--figure-dir "figures/with_adversaries/tradeoff_${1}/asr_${2}/lrs_${3}_${4}" \
		| tee "${log_dir}/lrs_${3}_${4}.txt"
}

run_without_adversaries() {
	log_dir="logs/eval_correlation/without_adversaries"
	mkdir -p $log_dir
	python -u eval_correlation.py \
		...
		--figure-dir "figures/without_adversaries/lrs_${1}_${2}" \
